...
struct ApplyFieldTests {
    ...
    @Test mutating func `Polar Coord: Default`() async throws {
    ...
    @Test mutating func `Polar Coord: (alt: 1, lat: 10, lon: 0)`() async throws {
    ...
    @Test mutating func `Polar Coord: (alt: 1, lat: 0, lon: 10)`() async throws {
    ...
    @Test mutating func `Polar Coord: (alt: 0.5, lat: 0, lon: 0)`() async throws {
        let particlePos = try await testAPI.gpu.testApplyFieldForPosition(alt: 0.5, lat: 0, lon: 0)
        let testPos: [Float] = [0, 0, 0]
        #expect(particlePos.elementsEqual(testPos, tolerance: tolerance))
    }
    
    @Test mutating func `Polar Coord: (alt: 1.5, lat: 0, lon: 0)`() async throws {
        let particlePos = try await testAPI.gpu.testApplyFieldForPosition(alt: 1.5, lat: 0, lon: 0)
        let testPos: [Float] = [0, 0, 0]
        #expect(particlePos.elementsEqual(testPos, tolerance: tolerance))
    }
    
    @Test mutating func `Test Velocity, Polar Coord: (alt: 1.5, lat: 0, lon: 0)`() async throws {
    ...
    @Test mutating func `PolarCoord: (alt: 1.5, lat: 90, lon: 0)`() async throws {
