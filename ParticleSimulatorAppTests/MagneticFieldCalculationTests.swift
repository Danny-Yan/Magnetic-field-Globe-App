...
struct ParticleSimulatorAppTests {
    ...
    private mutating func compareComponents(tolerance: Float = 0.005, alt: Double, latLon: [Double], day: Int = 1, month: Int = 1, year: Int = 2020 ) async throws -> ([Float],[Float],Float) {
        
        let expectedComponents = try await testAPI.external.testForComponent(
            alt: alt, lat: latLon[0], lon: latLon[1],
            day: day, month: month, year: year
        )
        
        let testComponents = try await testAPI.gpu.testForComponent(
            alt: alt, lat: latLon[0], lon: latLon[1],
            day: day, month: month, year: year,
            chosenModel: .WMM2020
        )
//        print("\(expectedComponents)")
    ...
    @Test mutating func `External Class, Location: Default, Date = Epoch`() async throws {
    ...
    @Test mutating func `Interal function, Location: Default, Date = Epoch`() async throws {
    ...
    @Test mutating func `External Class, Location: Flinder's Street, Date = Epoch`() async throws {
        printHeadlineSpacer(headline: "External")
    ...
    @Test mutating func `Internal Class, Location: Flinder's Street, Date = Epoch`() async throws {
        printHeadlineSpacer(headline: "External")
    ...
    // Comparative tests
    @Test mutating func `Location: Flinder's Street, Australia; Date = Epoch`() async throws {
    ...
    @Test mutating func `Location: Invalid Altitude, too high; Date = Epoch`() async throws {
    ...
    @Test mutating func `Location: Ouaguadougou, 500km altitude; Date = Epoch`() async throws {
    ...
    @Test mutating func `Location: Ouaguadougou, 1000km altitude; Date = Epoch`() async throws {
    ...
    @Test mutating func `Location: Ouaguadougou, 2000km altitude; Date = Epoch`() async throws {
    ...
    @Test mutating func `Location: Ouaguadougou, 10,000km altitude; Date = Epoch`() async throws {
        let (expectedOutcome, testOutcome, tolerance) = try await compareComponents(alt: 10000, latLon: [12.371918233382686, -1.5177017098658734])
        #expect(expectedOutcome.elementsEqual(testOutcome, tolerance: tolerance))
    }
    
    @Test mutating func `Location: Ouaguadougou, 100,000km altitude; Date = Epoch`() async throws {
    ...
    @Test mutating func `Location: Geographic South, 100,000km altitude; Date = Epoch`() async throws {
        let (expectedOutcome, testOutcome, tolerance) = try await compareComponents(alt: 100000, latLon: [-90, 0])
    ...
    @Test mutating func `WMM2025 tests`() async throws {
        ...
        for (index, entry) in testEntries.enumerated() {
            let dateTime = createDateFromFraction(yearFraction: Float(entry.timeFraction) )!
            ...
            let output = try! await testAPI.gpu.testForComponent(
                alt: entry.alt,
                lat: entry.lat,
                lon: entry.lon,
                day: day,
            ...
            // Loop through and test all entries in test file
            let testFraction = testOutput / output
...
//            #expect(output.elementsEqual(testOutput),
//                    """
//                    Row \(index) failed with entry: \(entryString)
//                        Expected: \(testOutput)
//                        Got: \(output)
//                    """
//            )
