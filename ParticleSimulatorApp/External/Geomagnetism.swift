...
© Deep Pradhan, 2017*/
class ExternalGeomagnetism {

	/** Initializes the instance without calculations*/
	init() {
		// Initialize constants
		maxord = ExternalGeomagnetism.MAX_DEG
		sp[0] = 0
		cp[0] = 1
		snorm[0] = 1
		pp[0] = 1
		...
		epoch = Double(ExternalGeomagnetism.WMM_COF[0].trimmingCharacters(in: .whitespaces).split(separator: " ")[0])!

		var tokens:[String.SubSequence], n:Int, m:Int, gnm:Double, hnm:Double, dgnm:Double, dhnm:Double

		for i in (1...ExternalGeomagnetism.WMM_COF.count - 1) {
			tokens = ExternalGeomagnetism.WMM_COF[i].trimmingCharacters(in: .whitespaces).split(separator: " ")
			n = Int(tokens[0])!
			m = Int(tokens[1])!
			gnm = Double(tokens[2])!
			hnm = Double(tokens[3])!
	...
	func calculate(longitude:Double, latitude:Double, altitude:Double? = 0, date:Date? = Date.init()) {
		let rlon:Double = longitude.toRadians,
				...
				a2:Double = ExternalGeomagnetism.WGS84_A * ExternalGeomagnetism.WGS84_A,
				b2:Double = ExternalGeomagnetism.WGS84_B * ExternalGeomagnetism.WGS84_B,
				c2:Double = a2 - b2,
				a4:Double = a2 * a2,
				b4:Double = b2 * b2,
				c4:Double = a4 - b4
		...
		let aor:Double = ExternalGeomagnetism.IAU66_RADIUS / r
